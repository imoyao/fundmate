  ...
  <div class="account-detail p-4 md:p-6 min-h-full" :style="{ backgroundColor: 'var(--bg-page)' }">
    ...
    <div class="mb-4 flex justify-between items-center">
      ...
      <div v-if="!isUnclassified && accountInfo" class="flex gap-2">
        ...
        <el-button @click="openBatchMigrateDialog" v-if="!isUnclassified && holdingsTotal > 0">
          <IconifyIconOffline icon="ep:share" class="mr-1" /> 批量迁移
        </el-button>
        <div class="flex items-center gap-1">
          <el-button v-if="!isUnclassified && accountInfo" type="danger" text @click="openDeleteDialog(accountInfo)">
    ...
    <template v-else>
        ...
        <div>
          ...
          <div class="flex items-center gap-2 mt-1">
            ...
            <template v-if="summaryData?.portfolio_name">
              <span class="text-xs" :style="{ color: 'var(--text-tertiary)' }"> · 组合: {{ summaryData.portfolio_name }}</span>
            </template>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div class="bg-white p-4 md:p-5 rounded-xl shadow-sm border border-gray-100 flex flex-col justify-between">
          ...
          <div class="text-2xl font-bold mt-2" :style="{ color: 'var(--color-primary)' }">
        ...
        <div class="bg-white p-4 md:p-5 rounded-xl shadow-sm border border-gray-100 flex flex-col justify-between">
          <div class="flex items-center gap-2 text-xs font-medium" :style="{ color: 'var(--text-secondary)' }">
            <IconifyIconOffline icon="ep:trend-charts" class="text-base" /> 持仓盈亏
          </div>
          <div class="mt-2 flex items-baseline gap-2">
            <span class="text-xl font-bold" :style="{ color: (summaryData?.position_pnl || 0) >= 0 ? 'var(--color-danger)' : 'var(--color-success)' }">
              {{ (summaryData?.position_pnl || 0) >= 0 ? '+' : '' }}{{ Math.abs(summaryData?.position_pnl || 0).toLocaleString() }}
            </span>
          </div>
        </div>
      ...
      <div class="grid grid-cols-2 gap-4 mb-6 bg-white p-4 rounded-xl shadow-sm border border-gray-100">
        ...
        <div class="flex flex-col justify-center border-r border-gray-100 pr-4">
          <div class="flex justify-between items-center mb-3">
            <div class="flex items-center gap-2 text-sm font-medium" :style="{ color: 'var(--text-secondary)' }">
              <IconifyIconOffline icon="ep:trend-charts" class="text-lg" /> 盈亏走势
            </div>
            <el-radio-group v-model="trendPeriod" size="small" @change="initLineChart">
          ...
          <div class="flex-1 min-h-[200px] w-full flex items-center justify-center" ref="lineChartRef">
        ...
        <div class="flex flex-col justify-center pl-2">
          <div class="flex items-center gap-2 mb-3 text-sm font-medium" :style="{ color: 'var(--text-secondary)' }">
      ...
      <el-card shadow="never">
        <el-tabs v-model="activeTab" @tab-change="onTabChange">
          ...
          <el-tab-pane label="持仓明细" name="holdings">
            <el-table :data="holdingsList" stripe @row-click="openPositionDrawer" size="default"
                ...
                <template #default="{ row }">
                  <div class="product-cell">
                    ...
                    <div class="product-code-row">
                      ...
                      <span v-if="row.type_label" class="type-tag-inline ml-2 px-2 py-0.5 rounded-full text-xs"
                        :style="{ backgroundColor: 'var(--bg-page)', color: 'var(--text-secondary)' }">{{ row.type_label }}</span>
                    </div>
                  </div>
                </template>
              </el-table-column>
              <el-table-column label="市值" width="110" align="right" sortable prop="market_value" show-overflow-tooltip>
              ...
              <el-table-column label="盈亏" width="110" align="right" sortable prop="pnl" show-overflow-tooltip>
                <template #default="{ row }">
                  <span :style="{ color: (row.pnl || 0) >= 0 ? 'var(--color-danger)' : 'var(--color-success)' }">
                    {{ (row.pnl || 0) >= 0 ? '+' : '' }}¥{{ Math.abs(row.pnl || 0).toLocaleString() }}
                  </span>
                </template>
              </el-table-column>
              <el-table-column label="盈亏率" width="100" align="right" sortable prop="pnl_rate" show-overflow-tooltip>
                <template #default="{ row }">
                  <span :style="{ color: (row.pnl_rate || 0) >= 0 ? 'var(--color-danger)' : 'var(--color-success)' }">
            ...
            <div class="flex justify-end mt-4">
              <el-pagination v-model:current-page="holdingsPage" :page-size="holdingsPageSize" layout="prev, pager, next"
                :total="holdingsTotal" small @current-change="loadHoldings" />
            </div>
          </el-tab-pane>

          <!-- 交易记录 Tab -->
          <el-tab-pane label="交易记录" name="transactions">
            <el-table :data="transactionsList" stripe size="default" :default-sort="{ prop: 'confirm_date', order: 'descending' }">
              <el-table-column prop="confirm_date" label="日期" width="110" sortable>
                <template #default="{ row }">{{ row.confirm_date?.slice(0, 10) }}</template>
              </el-table-column>
              ...
              <el-table-column label="操作" width="100" fixed="right" v-if="!isUnclassified">
                <template #default="{ row }">
                  <div class="flex items-center gap-1">
                    <el-button text size="small" @click.stop="openEditTxnDialog(row)">编辑</el-button>
                    <el-button text size="small" type="danger" @click.stop="confirmDeleteTxn(row)">删除</el-button>
                  </div>
                </template>
              </el-table-column>
            </el-table>
            <div v-if="transactionsTotal === 0" class="text-center py-8 text-gray-400">暂无交易记录</div>
            <div v-if="transactionsTotal > 0" class="flex justify-end mt-4">
              <el-pagination v-model:current-page="transactionsPage" :page-size="transactionsPageSize" layout="prev, pager, next"
                :total="transactionsTotal" small @current-change="loadTransactions" />
            </div>
          </el-tab-pane>
        </el-tabs>
      </el-card>
    ...
    <el-dialog v-model="migrateDialogVisible" title="迁移资产到其他账户" width="400px" destroy-on-close>
      <el-form label-width="80px">
        <el-form-item label="资产名称"><span>{{ migratingItem?.name || migratingItem?.symbol }}</span></el-form-item>
        <el-form-item label="目标账户">
          <el-select v-model="migrateTargetLedgerId" placeholder="选择同类型账户" class="w-full">
            <el-option v-for="ledger in sameTypeLedgers" :key="ledger.id" :label="ledger.name" :value="ledger.id" />
          </el-select>
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="migrateDialogVisible = false">取消</el-button>
        <el-button type="primary" @click="handleMigrate" :disabled="!migrateTargetLedgerId">确认迁移</el-button>
      </template>
    </el-dialog>

    <el-dialog v-model="batchMigrateVisible" title="批量迁移持仓" width="400px" destroy-on-close>
      <p class="mb-4" :style="{ color: 'var(--text-secondary)' }">将账户「{{ accountName }}」下的所有持仓迁移到目标账户。</p>
      <el-form label-width="80px">
        <el-form-item label="目标账户">
          <el-select v-model="batchTargetLedgerId" class="w-full" placeholder="选择同类型账户">
            <el-option v-for="ledger in sameTypeLedgers" :key="ledger.id" :label="ledger.name" :value="ledger.id" :disabled="ledger.id === Number(ledgerId)" />
          </el-select>
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <el-dialog v-model="editTxnDialogVisible" title="编辑交易" width="380px" destroy-on-close>
      <el-form :model="editTxnForm" label-width="80px">
        <el-form-item label="手续费"><el-input-number v-model="editTxnForm.fee" :precision="2" :min="0" class="w-full" /></el-form-item>
        <el-form-item label="备注"><el-input v-model="editTxnForm.notes" type="textarea" :rows="2" /></el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="editTxnDialogVisible = false">取消</el-button>
        <el-button type="primary" @click="handleUpdateTransaction">保存</el-button>
      </template>
    </el-dialog>
...
// 账户信息
const accountInfo = computed(() => ledgers.value.find((l: any) => String(l.id) === ledgerId.value) || null);
const accountName = computed(() => {
  if (targetAccountName.value) return targetAccountName.value;
  if (isUnclassified.value) return "未归置持仓";
  return accountInfo.value?.name || summaryData.value?.ledger_name || "账户详情";
});
...
// 关联现金账户列表
const cashLedgers = computed(() => ledgers.value.filter((l: any) => l.ledger_type === 'bank'));
...
async function loadHoldings(page = 1) {
  ...
  try {
    const res = await getLedgerPositions(Number(ledgerId.value), { page, per_page: holdingsPageSize });
...
const getCSSColor = (varName: string, fallback: string) => {
  ...
  console.log(`[ECharts颜色] 读取 ${varName}，结果：`, val || `❌ 没读到！使用后备色 ${fallback}`);
  return val || fallback;
};
...
function renderPieChart() {
  ...
  const pieData = Array.from(dataMap.entries()).map(([name, value]) => ({ name, value }));
  ...
  const option = {
    ...
    series: [{
      ...
      label: isDataEmpty
        ? { show: true, position: 'center', formatter: '暂无配置', color: '#999', fontSize: 12 }
        : { show: false },
...
async function loadTransactions(page = 1) {
  ...
  try {
    const res = await getLedgerTransactions(Number(ledgerId.value), { page, per_page: transactionsPageSize });
    const result = (res as any)?.data;
...
function onTabChange(tabName: string) {
  if (tabName === "holdings" && holdingsList.value.length === 0) {
    loadHoldings();
  } else if (tabName === "transactions" && transactionsList.value.length === 0) {
    loadTransactions();
  }
}

function txnTypeLabel(type: string) {
  const map: Record<string, string> = { buy: "买入", sell: "卖出", dividend: "分红", deposit: "存入", withdraw: "取出" };
...
async function handleMigrate() {
  ...
  try {
    if (migratingItem.value.id > 100000) {
      ...
      await updateLedgerPosition(Number(ledgerId.value), migratingItem.value.id, { account_name: ledger.name });
    }
...
async function handleBatchMigrate() {
  ...
  try {
    await migrateLedgerPositions(Number(ledgerId.value), batchTargetLedgerId.value);
...
async function handleAssign(itemId: number) {
  ...
  try {
    if (itemId > 100000) {
      ...
      await updateLedgerPosition(Number(ledgerId.value), itemId, { account_name: ledger.name });
    }
...
async function handleUpdate() {
  if (!editForm.value.name.trim()) { ElMessage.warning("名称不能为空"); return; }
  saving.value = true;
  try {
...
async function handleUpdateTransaction() {
  try {
    await updateLedgerTransaction(Number(ledgerId.value), editTxnForm.value.id, {
