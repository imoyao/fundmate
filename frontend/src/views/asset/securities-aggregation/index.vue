<template>
  <div
    class="securities-aggregation p-4 md:p-6 min-h-full"
  ...
  >
    ...
    <div class="mb-6 flex flex-wrap justify-between items-center gap-3">
      ...
      <el-radio-group v-model="dimension" @change="onDimensionChange">
        ...
        <el-radio-button label="institution">按券商</el-radio-button>
    ...
    <div v-else class="overview-card table-card">
      ...
      >
        ...
        <el-table-column label="证券代码" prop="symbol" min-width="120" />
        <el-table-column label="证券名称" prop="name" min-width="200" />
      ...
      >
        ...
        <el-table-column label="券商" min-width="200">
          <template #default="{ row }">
            {{ institutionLabel(row.key) }}
          </template>
        </el-table-column>
        <el-table-column label="证券数" min-width="100" align="right">
...
import {
  ...
  type SecuritiesAggregationResult,
...
defineOptions({ name: "securities-aggregation" });

const dimension = ref<FundAggregationDimension>("product");
const result = ref<SecuritiesAggregationResult | null>(null);
...
function institutionLabel(key: number | "unknown"): string {
  return key === "unknown" ? "未关联券商" : `券商 #${key}`;
}
...
function rowKey(row: AggRow): string {
  if (dimension.value === "product") {
    return (row as FundAggregationProductGroup).symbol;
  }
  return String((row as FundAggregationInstitutionGroup | FundAggregationAppGroup).key);
}
...
async function load() {
  ...
  errorMsg.value = "";
  try {
    const res = await getSecuritiesAggregation(dimension.value);
