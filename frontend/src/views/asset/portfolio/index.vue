<!-- portfolio/index.vue -->
<template>
  <div class="portfolio-page p-4 md:p-6 min-h-full" :style="{ backgroundColor: 'var(--bg-page)' }">
    <div class="mb-6 flex justify-between items-center">
      <div>
        <h2 class="text-2xl font-bold" :style="{ color: 'var(--text-primary)' }">投资组合</h2>
        <p class="text-sm mt-1" :style="{ color: 'var(--text-tertiary)' }">管理你的投资策略组合，关联账户进行收益分析</p>
      </div>
    ...
    <!-- 空状态 -->
    <div v-else-if="portfolios.length === 0" class="text-center py-20" :style="{ color: 'var(--text-tertiary)' }">
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
      ...
      >
        <div class="flex items-center justify-between mb-3">
          <h3 class="font-semibold text-base" :style="{ color: 'var(--text-primary)' }">{{ item.name }}</h3>
          <el-popconfirm
        ...
        <div class="flex flex-col gap-2">
          <div class="flex items-center gap-2">
            <span class="text-xs px-2 py-0.5 rounded-full" :style="{ backgroundColor: 'var(--bg-page)', color: 'var(--text-secondary)' }">
              {{ item.purpose || '未设定目的' }}
            </span>
          </div>
    ...
    <!-- 仅保留创建对话框 -->
    <el-dialog v-model="dialogVisible" title="新建组合" width="500px" destroy-on-close>
      <el-form :model="form" label-width="90px">
        ...
        <el-form-item label="目标收益率">
          <el-input-number v-model="form.target_return" :min="0" :max="100" :precision="2" controls-position="right" class="w-full" placeholder="年化目标（%）" />
        </el-form-item>
        <el-form-item label="目标金额">
          <el-input-number v-model="form.target_amount" :min="0" :precision="2" controls-position="right" class="w-full" placeholder="目标金额" />
        </el-form-item>
        <el-form-item label="目标日期">
          <el-date-picker v-model="form.target_date" type="date" placeholder="选择日期" class="w-full" value-format="YYYY-MM-DD" />
        </el-form-item>
        <el-form-item label="基准指数">
          <el-select v-model="form.benchmark" class="w-full" clearable filterable allow-create>
