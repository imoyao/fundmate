    ...
    <template #footer>
      ...
      <div v-else-if="currentStep === 2" class="preview-table">
        ...
        >
          ...
          <el-table-column label="配置目标" width="120">
            <template #default="{ row }">
              <el-select
...
function onLedgerSelected(id: number) {
  // 用户选择账户后的回调，可选：同步默认配置目标
  ledgerTouched.value = false; // 选择后隐藏错误提示
}
...
async function createLedger() {
  ...
  try {
    ...
    await fetchLedgers();
    const newId = (res as any).data?.id; // 获取新建账户的 ID
    if (newId) {
      selectedLedgerId.value = newId;    // 自动选中
    }
...
async function handleUpload(options: UploadRequestOptions) {
  ...
  try {
    ...
    previewData.value.forEach(row => {
      if (!row.allocation) row.allocation = 'longterm'; // 默认值
    });
    ...
    const ledger = ledgers.value.find(l => l.id === selectedLedgerId.value)?.name || '默认账户';

    previewData.value = addRowKeys(rawData).map(row => ({
      ...row,
      account_name: row.account_name && row.account_name !== '默认证券账户' ? row.account_name : ledger,
      allocation: row.allocation || ledgers.value.find(l => l.id === selectedLedgerId.value)?.default_allocation || 'longterm'
    }));
    // 初始智能填充：对价格缺失但数量和金额都有的行计算参考价格
    previewData.value.forEach(row => {
      row.account_name = row.account_name && row.account_name !== '默认证券账户' ? row.account_name : ledger;
      if (!row.allocation) {
        row.allocation = ledgers.value.find(l => l.id === selectedLedgerId.value)?.default_allocation || 'longterm';
      }
