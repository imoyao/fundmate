    ...
    filteredList(state): FavoriteItem[] {
      ...
      if (state.search) {
        ...
        result = result.filter(
          item => item.symbol.toLowerCase().includes(kw) || item.display_name.toLowerCase().includes(kw)
        );
      }
      // 排序
      if (state.sort === 'favorite_at') {
        result.sort((a, b) => new Date(b.favorite_at || '').getTime() - new Date(a.favorite_at || '').getTime());
