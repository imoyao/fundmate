...
4. **默认列预算（#1285，2026-09-10）**：自选表格**默认可见列总宽 ≤ 1040px**（按最受约束的主流笔记本 1366×768：内容区 = 1366 − 侧栏 240 − 内边距）。超出预算的列放「管理 → 列设置」开启；**用户自行加列导致的横向滚动属正常，不得以"藏列"为手段规避溢出**。
5. **混合视图 / 品类视图列集（#1285）**：列定义带 `scope` —— `mixed`＝混合视图通用列（名称 / 价格·净值 / 涨跌幅 / 操作），`category`（默认）＝品类专属列（仅品类视图显示；`appliesTo` 限定适用 `asset_type`；用户在列设置显式开启后任何视图都显示）。**混合视图只显示通用列**（差异化列在混合视图恒为 `—`、无意义）；**品类视图**（类型筛选命中单一品类）显示该品类完整列组。
