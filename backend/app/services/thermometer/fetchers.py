...
class SelfCalcFetcher(BaseFetcher):
    ...
    def fetch(self) -> Optional[Dict[str, Any]]:
        try:
            ...
            # 1. 沪深300 PE（滚动市盈率）—— 慢速接口，走 12h 缓存
            pe_df = _cached('self_calc_pe', 12 * 3600, lambda: ak.stock_index_pe_lg(symbol='沪深300'))
            ...
            # 2. 10Y 国债收益率（缓存）
            bond_df = _cached('self_calc_bond', 12 * 3600, lambda: ak.bond_zh_us_rate())
            ...
            # 3. CPI 同比（最慢的接口，必走缓存）
            cpi_df = _cached('self_calc_cpi', 12 * 3600, lambda: ak.macro_china_cpi())
...
class JiucaishuoFetcher(BaseFetcher):
    """韭圈儿：恐惧贪婪指数 + 中长期温度（需 Playwright 浏览器）。

    返回 ``{'fear': {...}, 'medium': {...}, 'updated_at': ...}``，
    单个子源缺失时对应字段为 ``None``。
    """
    ...
    def fetch(self) -> Optional[Dict[str, Any]]:
        ...
        result = {'fear': None, 'medium': None, 'updated_at': None}
        try:
            with sync_playwright() as p:
                browser = p.chromium.launch(headless=True, args=['--no-sandbox', '--disable-dev-shm-usage'])
                page = browser.new_page(user_agent=USER_AGENT)
                page.goto(JIUCASHUO_URL, wait_until='networkidle', timeout=30000)
