...
def get_strategy_overview():
    ...
    with get_db() as db:
        ...
        # 3. 所有策略标签
        tags = db.query(StrategyTag).order_by(StrategyTag.created_at.asc()).all()
...
def create_tag(json_data: StrategyTagCreate):  # 类型注解为模型实例
    with get_db() as db:
        existing = db.query(StrategyTag).filter(StrategyTag.name == json_data.name).first()
        if existing:
            return jsonify({'data': None, 'message': '标签名称已存在'}), 409

        tag = StrategyTag(name=json_data.name)
        db.add(tag)
        db.commit()
        db.refresh(tag)
...
def delete_tag(tag_id: int):
    """删除策略标签（级联删除关联）"""
    with get_db() as db:
        tag = db.query(StrategyTag).get(tag_id)
        if not tag:
            abort(404, '标签不存在')
        db.delete(tag)
        db.commit()
