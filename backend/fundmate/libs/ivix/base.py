    ...
    if put.empty:
        ...
        try:
            if len(put_all.loc[ff_idx].CLOSE.values) > 1:
                put['CLOSE'].iloc[-1] = (put_all.loc[ff_idx].CLOSE.values[1] + call_all.loc[ff_idx].CLOSE.values[
                    0]) / 2.0
        except:
...
if __name__ == '__main__':
    ...
    Line().add_xaxis(attr).add_yaxis("中证指数发布", true_ivix[u'收盘价(元)'].tolist(), is_smooth=True,
                                     markpoint_opts=['average', 'max']).add_yaxis("公式计算数据", ivix, is_smooth=True,
                                                                                  markline_opts=['max']).render(
        f"{current_path}/vix.html")
