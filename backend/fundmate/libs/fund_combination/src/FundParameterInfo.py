...
class FundInfo:
    ...
    def update_fund_info_by_chenxing(self):
        ...
        # 获取基金三个月内的最大回撤
        sleep(3)
        self.three_month_retracement = float(
            chrome_driver.find_element_by_id("qt_worst").find_element_by_class_name("r3").text)
        # 获取股票总仓位、前十大持仓、债券总仓位、前五大持仓
        sleep(3)
        self.stock_total_position["stock_total_position"] = float(
            chrome_driver.find_element_by_class_name("stock").text) / 100  # 股票的总仓位
        sleep(3)
        self.bond_total_position["bond_total_position"] = float(
            chrome_driver.find_element_by_class_name("bonds").text) / 100  # 债券的总仓位
        sleep(3)
        self.stock_total_position["ten_stock_position"] = float(
            re.findall(r"\d+\.?\d*",
                       chrome_driver.find_element_by_id("qt_stocktab").text).pop(0)) / 100  # 十大股票仓位
        sleep(3)
        self.bond_total_position["five_bond_position"] = float(
            re.findall(r"\d+\.?\d*",
                       chrome_driver.find_element_by_id("qt_bondstab").text).pop(0)) / 100  # 五大债券仓位
        # 获取标准差
        sleep(3)
