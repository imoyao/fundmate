...
def get_quarter_index(input_date):
    ...
    input_date_strptime = datetime.datetime.strptime(year + '-' + input_date, '%Y-%m-%d')
    index = 1
    for idx in range(len(boundary_date_list)):
        join_date = year + '-' + boundary_date_list[idx]
        season_date_strptime = datetime.datetime.strptime(join_date, '%Y-%m-%d')
...
def get_stock_market(stock_code):
    if bool(re.search("^\d{5}$", stock_code)):
        return '港股'
    elif bool(re.search("^\d{6}$", stock_code)) and bool(
...
def update_xlsx_file(path, df_data, sheet_name):
    if os.path.exists(path):
        ...
        if len(book.sheetnames) == 0:
            df_data.to_excel(path, sheet_name=sheet_name)
            return
        else:
            writer.book = book
