...
def identify_verification_code(chrome_driver, id="checkcodeImg"):
    ...
    try:
        ...
        if url:
            ...
            identify_code = pytesseract.image_to_string(image, config=custom_oem_psm_config)
...
def mock_login_site(chrome_driver, site_url, redirect_url=None):
    ...
    while count < 10 and flag:
        ...
        try:
            message_box = chrome_driver.find_element_by_id('message-container')
            flag = message_box.is_displayed()
            if flag:
                close_btn = message_box.find_element(By.CLASS_NAME, "modal-close")
                close_btn.click()
                time.sleep(1)
            print('flag', flag)
...
def login_morning_star(redirect_url, is_cookies_login=False):
    ...
    if cookie_str:
        ...
        login_status = mock_login_site(chrome_driver, login_url, redirect_url)
        if login_status:
            print('login success')
        else:
            print('login fail')
