...
def acquire_fund_quarter():
    ...
    def crawlData(start, end):
        ...
        while (page_start < end):
            results = each_fund_query.select_quarter_fund(page_start, page_limit)
            for record in results:
                sleep(1)
                each_fund = FundSpider(record[0], record[1], record[2], chrome_driver)
                is_normal = each_fund.go_fund_url()
                # 是否能正常跳转到基金详情页，没有的话，写入csv,退出当前循环
                if is_normal == False:
                    # error_funds.append(each_fund.fund_code)
                    fund_infos = [
                        each_fund.fund_code, each_fund.morning_star_code, each_fund.fund_name, record[3], page_start,
                        '页面跳转有问题'
                    ]
                    output_line = ', '.join(str(x) for x in fund_infos) + '\n'
                ...
                if quarter_index != each_fund.quarter_index:
                    print('quarter_index', quarter_index, each_fund.update_date, each_fund.fund_code,
                          each_fund.fund_name)
                    continue
                ...
                if each_fund._is_trigger_catch == True:
                    fund_infos = [
                        each_fund.fund_code, each_fund.morning_star_code, each_fund.fund_name, record[3],
                        each_fund.stock_position['total'], page_start, each_fund._catch_detail
                    ]
                    output_line = ', '.join(str(x) for x in fund_infos) + '\n'
                ...
                # 入库十大股票持仓
                stock_position_total = each_fund.stock_position.get('total', '0.00')
                ...
                if each_fund.fund_name.endswith('A'):
                    ...
                    results = each_fund_query.select_similar_fund(similar_name)  # 获取查询的所有记录
