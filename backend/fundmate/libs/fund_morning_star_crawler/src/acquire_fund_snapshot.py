...
from utils.index import bootstrap_thread, get_star_count
    ...
    def _predicate(driver):
        try:
            ...
            if int(element_text) != int(text):
                # 跳转指定的js执行代码
                js_content = "javascript:__doPostBack('ctl00$cphMain$AspNetPager1','{}')".format(text)
...
def get_fund_list(page_index):
    ...
    page_total = math.ceil(
        int(chrome_driver.find_element_by_xpath('/html/body/form/div[8]/div/div[4]/div[3]/div[2]/span').text) /
        page_count)
    ...
    while page_index <= page_total:
        ...
        # 判断是否最后一页
        num = (remainder + 2) if page_index > (page_total - remainder) else 12
        xpath_str = '/html/body/form/div[8]/div/div[4]/div[3]/div[3]/div[1]/a[%s]' % (num)
        ...
        for i in range(len(class_list)):
            for tr in bs.find_all('tr', {'class': class_list[i]}):
                ...
                # 从href中匹配出晨星专属代码
                current_morning_code = re.findall(r'(?<=/quicktake/)(\w+)$', code_a_element.get('href')).pop(0)
                # 晨星基金专属晨星码
