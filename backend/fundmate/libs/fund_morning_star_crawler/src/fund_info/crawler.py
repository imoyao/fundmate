...
from utils.index import get_last_quarter_str, get_quarter_index, get_star_count
...
class FundSpider:
    ...
    def get_element_text_by_class_name(self, class_name, parent_id):
        try:
            text = self._chrome_driver.find_element_by_id(parent_id).find_element_by_class_name(class_name).text
            return text if text != '-' else None
        except NoSuchElementException:
            self._is_trigger_catch = True
            self._catch_detail = parent_id + '-' + class_name
            print('error_fund_info:', self.fund_code, '-', self.morning_star_code, self.stock_position["total"],
                  class_name)
    ...
    def get_element_text_by_id(self, id):
        try:
            text = self._chrome_driver.find_element_by_id(id).text
            return text if text != '-' else None
        except NoSuchElementException:
            self._is_trigger_catch = True
            self._catch_detail = id
            print('error_fund_info:', self.fund_code, '-', self.morning_star_code, self.stock_position["total"], id)
    ...
    def get_element_text_by_xpath(self, xpath, parent_id=None, parent_el=None):
        try:
            text = '-'
            if parent_el == None:
                text = self._chrome_driver.find_element_by_xpath(
                    xpath).text if parent_id == None else self._chrome_driver.find_element_by_id(
            ...
            print('error_fund_info:', self.fund_code, '-', self.morning_star_code, self.stock_position["total"], xpath)
            file_name = './abnormal/' + \
    ...
    def get_fund_base_info(self):
        # 基金分类
        self.fund_cat = self.get_element_text_by_class_name("category", 'qt_base')
        # 成立时间
        self.found_date = self.get_element_text_by_class_name("inception", 'qt_base')
        # 基金公司
        self.company = self.get_element_text_by_xpath("//ul[@id='qt_management']/li[4]/span[@class='col2 comp']/a",
                                                      'qt_management')
    ...
    def get_fund_manager_info(self):
        try:
            # 基金经理
            manager_ele = self._chrome_driver.find_element_by_id('qt_manager').find_element_by_xpath("ul")
            manager_name = manager_ele.find_element_by_xpath("li[@class='col1']/a").text
            manager_id = re.findall(
                r'(?<=managerid=)(\w+)$',
                manager_ele.find_element_by_xpath("li[@class='col1']/a").get_attribute('href')).pop(0)
            manager_start_date = manager_ele.find_element_by_xpath("li[@class='col1']/i").text[0:10]
            manager_brife = manager_ele.find_element_by_xpath("li[@class='col2']").text
            ...
            self._is_trigger_catch = True
            print('error_fund_info:', self.fund_code, '-', self.morning_star_code, 'get_fund_manager_info')
    ...
    def get_fund_morning_rating(self):
        try:
            ...
            rating_3_src = qt_el.find_element_by_xpath("//li[@class='star3']/img").get_attribute('src')
            rating_5_src = qt_el.find_element_by_xpath("//li[@class='star5']/img").get_attribute('src')
            rating_10_src = qt_el.find_element_by_xpath("//li[@class='star10']/img").get_attribute('src')
            ...
            self._is_trigger_catch = True
            print('error_fund_info:', self.fund_code, '-', self.morning_star_code, 'get_fund_morning_rating')
    ...
    def get_fund_qt_rating(self):
        try:
            ...
            rating_2_src = qt_el.find_element_by_xpath("//li[5]/img").get_attribute('src')
            rating_3_src = qt_el.find_element_by_xpath("li[6]/img").get_attribute('src')
            rating_5_src = qt_el.find_element_by_xpath("li[7]/img").get_attribute('src')
            rating_10_src = qt_el.find_element_by_xpath("li[8]/img").get_attribute('src')
            # //*[@id="qt_rating"]/li[6]/img
            rating_2 = re.findall(r"\d(?:stars\.gif)$", rating_2_src)[0][0]
            rating_3 = re.findall(r"\d(?:stars\.gif)$", rating_3_src)[0][0]
            rating_5 = re.findall(r"\d(?:stars\.gif)$", rating_5_src)[0][0]
            rating_10 = re.findall(r"\d(?:stars\.gif)$", rating_10_src)[0][0]
            ...
            self._is_trigger_catch = True
            print('error_fund_info:', self.fund_code, '-', self.morning_star_code, 'get_fund_qt_rating')
            file_name = './abnormal/qt_rating-' + self.fund_code + "-no_such_element.png"

    def get_fund_season_info(self):
        # 总资产  TODO: 增加一个数据更新时间field
        self.total_asset = self.get_element_text_by_class_name("asset", 'qt_base')
        # 投资风格
        self.investname_style = self.get_element_text_by_class_name('sbdesc', 'qt_base')
        # 最差三个月回报
        self.three_month_retracement = self.get_element_text_by_class_name("r3", 'qt_worst')
        # 最差六个月回报
        self.june_month_retracement = self.get_element_text_by_class_name("r6", 'qt_worst')
        # 获取股票总仓位、前十大持仓、债券总仓位、前五大持仓 TODO: 增加一个数据更新时间field
        total = self.get_element_text_by_class_name("stock", 'qt_asset')
        self.stock_position["total"] = total if total != None else '0.00'
        # self.total["total"] = float(
        #     total) / 100 if total != '-' else None  # 股票的总仓位
        self.bond_position["total"] = self.get_element_text_by_class_name("bonds", 'qt_asset')
        ...
        if ten_stock_position_text != None or ten_stock_position_text != '-':
            ten_stock_position_list = re.findall(r"\d+\.?\d*", ten_stock_position_text)
        ...
        if five_bond_position_text != None or five_bond_position_text != '-':
            five_bond_position_list = re.findall(r"\d+\.?\d*", five_bond_position_text)
        ...
        standard_deviation = self.get_element_text_by_xpath('li[16]', 'qt_risk')
        if standard_deviation != None:
            self.risk_assessment["standard_deviation"] = standard_deviation
            # 获取风险系数
            risk_coefficient = self.get_element_text_by_xpath('li[23]', 'qt_risk')
            self.risk_assessment["risk_coefficient"] = risk_coefficient
            # 获取夏普比
            sharpby = self.get_element_text_by_xpath('li[30]', 'qt_risk')
            self.risk_assessment["sharpby"] = sharpby
            # 获取阿尔法
            alpha = self.get_element_text_by_xpath('li[5]', 'qt_riskstats')
            self.risk_statistics["alpha"] = alpha
            # 获取贝塔
            beta = self.get_element_text_by_xpath('li[8]', 'qt_riskstats')
            self.risk_statistics["beta"] = beta
            # 获取R平方
            r_square = self.get_element_text_by_xpath('li[11]', 'qt_riskstats')
    ...
    def get_asset_composition_info(self):
        # 判断是否含有股票持仓
        li_elements = self._chrome_driver.find_element_by_id('qt_stock').find_elements_by_xpath("li")
        for index in range(4, len(li_elements) - 1, 4):
            ...
            stock_market = None if len(stock_base) == 1 else stock_base.pop()
            #纠正港股市场及代码 -- 港股号位是5位
            ...
            temp_stock_info['stock_market'] = stock_market.replace(' ', '') if stock_market else stock_market
    ...
    def get_quarter_index(self):
        update_date = self.get_element_text_by_class_name("date4", 'aspnetForm')
        if (update_date == None):
            self._chrome_driver.refresh()
            print('wait:fund_code', self.fund_code)
            sleep(9)
