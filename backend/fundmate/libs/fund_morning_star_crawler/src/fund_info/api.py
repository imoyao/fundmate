...
class FundApier:
    ...
    def __init__(self, code, *, end_date=None, platform='ai_fund'):
        ...
        self.file_dir = os.getcwd() + '/output/json/' + self.platform + \
            '/' + self.cur_date + '/'
        self.file_path = '{file_dir}{fund_code}-{end_date}-base.json'.format(file_dir=self.file_dir,
                                                                             fund_code=code,
    ...
    def get_asset_from_json(self):
        with open(self.file_path) as json_file:
            ...
            asset_str = my_data.get('asset') if self.platform == 'ai_fund' else my_data.get('FundScope')[0:-1]
            try:
    ...
    def get_base_info_ai(self):
        url = "http://fund.10jqka.com.cn/data/client/myfund/{0}".format(self.fund_code)
    ...
    def get_base_info_zh(self):
        url = "https://www.myfund.com/webinterface/Bamboo.ashx?command={0}".format('fundInfoHead_NEW')
    ...
    def get_analyse_info_zh(self):
        url = "https://www.myfund.com/webinterface/Bamboo.ashx?command={0}".format('singlefundAnalyse')
        headers = {'Content-Type': 'application/x-www-form-urlencoded;charset=utf-8'}
        payload = {
            'fundcode': self.fund_code,
        }
    ...
    def write_info_in_json(self, end_date, json_data):
        filename = '{fund_code}{end_date}-base.json'.format(fund_code=self.fund_code,
