...
class BaseQuery(BaseModel):
    ...
    def format_sql(self, table_name, field_name, field_dict, prefix="AND"):
        ...
        sql_str = '{prefix} {table_name}.{field_name} {operator} {value}'.format(**format_dict)
        return sql_str
...
class FundQuery(BaseQuery):
    ...
    def get_fund_from_snapshot_table_no_exist(self, page_start, page_limit):
        ...
        self.cursor.execute(sql, [page_start, page_limit])  # 执行sql语句
    ...
    def select_quarter_fund(self, page_start, page_limit):
        ...
        self.cursor.execute(sql, [self.quarter_date, self.quarter_index, page_start, page_limit])  # 执行sql语句
    ...
    def select_certain_condition_funds(self,
                                       *,
                                       quarter_index=None,
                                       morning_star_rating_5=None,
                                       morning_star_rating_3=None,
                                       manager_start_date=None,
                                       stock_position_total=None,
                                       stock_position_ten=None,
                                       **rest_dicts):
        print("rest_dicts", rest_dicts)
        if quarter_index == None:
            quarter_index = self.quarter_index

        morning_star_rating_3_sql = self.format_sql('a', 'morning_star_rating_3', morning_star_rating_3)

        morning_star_rating_5_sql = self.format_sql('a', 'morning_star_rating_5', morning_star_rating_5)

        manager_start_date_sql = self.format_sql('a', 'manager_start_date', manager_start_date)

        stock_position_total_sql = self.format_sql('a', 'stock_position_total', stock_position_total)

        stock_position_ten_sql = self.format_sql('a', 'stock_position_ten', stock_position_ten)

        risk_assessment_sharpby = rest_dicts.get('risk_assessment_sharpby')

        risk_assessment_sharpby_sql = self.format_sql('a', 'risk_assessment_sharpby', risk_assessment_sharpby)
        ...
        risk_rating_2_sql = self.format_sql('a', 'risk_rating_2', risk_rating_2)

        risk_rating_3 = rest_dicts.get('risk_rating_3')

        risk_rating_3_sql = self.format_sql('a', 'risk_rating_3', risk_rating_3)

        risk_rating_5 = rest_dicts.get('risk_rating_5')

        risk_rating_5_sql = self.format_sql('a', 'risk_rating_5', risk_rating_5)
