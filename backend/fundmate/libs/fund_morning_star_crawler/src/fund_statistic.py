...
def get_fund_code_pool(condition_dict):
    ...
    morning_star_rating_5_condition = {'value': 4, 'operator': '>='}
    morning_star_rating_3_condition = {'value': 5, 'operator': '='}
    # last_year_time = time.localtime(time.time() - 365 * 24 * 3600)
    # last_year_date = time.strftime('%Y-%m-%d', last_year_time)
    fund_code_pool = each_statistic.select_fund_pool(**condition_dict, )
...
def stocks_compare(stock_list, *, market=None, quarter_index=None, is_A_stock=None):
    ...
    for stock in stock_list:
        ...
        if len(target_loc) == 1:
            ...
            last_holder_asset = round(decimal.Decimal(target_loc[col_target].iloc[0]), 4)
        diff_holder_count = holder_count - last_holder_count
        diff_holder_asset = holder_asset - last_holder_asset
        diff_holder_count_percent = '{:.2%}'.format(diff_holder_count /
                                                    last_holder_count) if last_holder_count != 0 else "+∞"
        diff_holder_asset_percent = '{:.2%}'.format(diff_holder_asset /
        ...
        item_tuple = [
            stock_code, stock_name, holder_count, last_holder_count, diff_holder_count, diff_holder_count_percent,
            flag_count, holder_asset, last_holder_asset, diff_holder_asset, diff_holder_asset_percent, flag_asset
        ]
        if is_A_stock:
...
def select_condition_stocks_rank(each_statistic=None, *, quarter_index=None):
    ...
    stock_top_list = each_statistic.all_stock_fund_count(quarter_index=quarter_index,
...
def t100_stocks_rank(each_statistic=None, *, quarter_index=None):
    ...
    columns = [
        '代码', '名称', quarter_index + '持有数量（只）', last_quarter_index + '持有数量（只）', '持有数量环比', '持有数量环比百分比', '持有数量升或降',
        quarter_index + '持有市值（亿元）', last_quarter_index + '持有市值（亿元）', '持有市值环比', '持有市值环比百分比', '持有市值升或降'
    ]

    stock_top_list = each_statistic.all_stock_fund_count(quarter_index=quarter_index, filter_count=80)
...
def all_stocks_rank(each_statistic=None):
    ...
    stock_top_list = each_statistic.all_stock_fund_count(quarter_index=quarter_index, filter_count=0)
    ...
    for stock_name_code in stock_top_list:
        ...
        if bool(re.search("^\d{5}$", stock_code)):
            ...
            a_condition = bool(
                re.search("^(00(0|1|2|3)\d{3})|(30(0|1)\d{3})|(60(0|1|2|3|5)\d{3})|68(8|9)\d{3}$", stock_code))
...
def all_stock_holder_detail(each_statistic=None, *, quarter_index=None, threshold=0):
    ...
    stock_list = each_statistic.all_stock_fund_count_and_details(quarter_index=quarter_index, filter_count=threshold)
...
def get_special_fund_code_holder_stock_detail(each_statistic=None, quarter_index=None):
    ...
    columns = [
        '基金代码', '基金名称', '基金类型', '基金经理', '基金总资产（亿元）', '基金股票总仓位', '十大股票仓位', '股票代码', '股票名称', '所占仓位', '所处仓位排名', '三级行业',
        '二级行业', '一级行业'
    ]
