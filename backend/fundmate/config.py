...
class Config:
    ...
    JWT_ACCESS_LIFESPAN_TYPE = env.str('JWT_ACCESS_LIFESPAN_TYPE', 'hours')
    JWT_ACCESS_LIFESPAN_LENGTH = env.int('JWT_ACCESS_LIFESPAN_LENGTH', 24)
    JWT_ACCESS_LIFESPAN = {JWT_ACCESS_LIFESPAN_TYPE: JWT_ACCESS_LIFESPAN_LENGTH}
    JWT_REFRESH_LIFESPAN_TYPE = env.str('JWT_REFRESH_LIFESPAN_TYPE', 'days')
    JWT_REFRESH_LIFESPAN_LENGTH = env.int('JWT_REFRESH_LIFESPAN_LENGTH', 30)
    JWT_REFRESH_LIFESPAN = {JWT_REFRESH_LIFESPAN_TYPE: JWT_REFRESH_LIFESPAN_LENGTH}
    PRAETORIAN_CONFIRMATION_SUBJECT = env.str('PRAETORIAN_CONFIRMATION_SUBJECT', '请确认您的注册请求：')
    # FIXME: 需要替换为真实的uri
    PRAETORIAN_CONFIRMATION_URI = env.str('PRAETORIAN_CONFIRMATION_URI')
    '''
    # 旧版本
    import random
    import string
    ...
    PRAETORIAN_CONFIRMATION_SENDER = env.str('PRAETORIAN_CONFIRMATION_SENDER', MAIL_USERNAME)
    # redis 配置
