...
def test_flask_spec_format(app, cli_runner, format, tmp_path):
    ...
    file_result = cli_runner.invoke(spec_command, ['--format', format, '--output', str(local_spec_path)])
    if format == 'json':
...
def test_flask_spec_indent(cli_runner, indent, tmp_path):
    ...
    file_result = cli_runner.invoke(spec_command, ['--indent', indent, '--output', str(local_spec_path)])
    if indent == 0:
