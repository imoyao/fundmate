...
class TestXIRRPlus:
    ...
    def test_xirr(valuesPerDateString, expected):
        valuesPerDate = {
            ...
            for k, v in valuesPerDateString.items()
        }
        actual = rr.XIRRPlus.xirr(valuesPerDate)
        if expected:
    ...
    def test_clean_xirr(valuesPerDateString, expected):
        valuesPerDate = {
            datetime.fromisoformat(k).date(): v
            for k, v in valuesPerDateString.items()
        }
        actual = rr.XIRRPlus.clean_xirr(valuesPerDate)
        if expected:
            assert round(actual, 4) == expected
        else:
            assert actual == expected
    ...
    def test_xnpv(valuesPerDateString, rate, expected):
        ...
        actual = rr.XNPV.xnpv(valuesPerDate, rate)
        if expected:
