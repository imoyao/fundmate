import pytest
from apiflask import auth_required, doc, input, output
from apiflask.schemas import EmptySchema, http_error_schema
from apiflask.security import HTTPBasicAuth
from openapi_spec_validator import validate_spec

from .schemas import FooSchema, HTTPErrorSchema, ValidationErrorSchema
...
def test_response_description_config(app, client):
    ...
    assert rv.json['paths']['/baz']['get']['responses']['204']['description'] == 'Success'
    assert rv.json['paths']['/spam']['get']['responses']['206']['description'] == 'Success'
    assert rv.json['paths']['/eggs/{id}']['get']['responses']['404']['description'] == 'Egg not found'
...
def test_validation_error_status_code_and_description(app, client):
    ...
    assert rv.json['paths']['/foo']['post']['responses']['422']['description'] == 'Bad'


@pytest.mark.parametrize('schema', [http_error_schema, ValidationErrorSchema])
def test_validation_error_schema(app, client, schema):
    ...
    assert rv.json['paths']['/foo']['post']['responses']['400']['description'] == 'Validation error'
...
@pytest.mark.parametrize('schema', [http_error_schema, HTTPErrorSchema])
