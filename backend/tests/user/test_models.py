...
class TestUser:
    ...
    def test_lookup(self, app, create_test_user):
        with app.app_context():
            user = User.lookup('foo@bar.com')
            assert user

    def test_check_password(self, app, create_test_user):
        with app.app_context():
            user = User.lookup('foo@bar.com')
    ...
    def test_avatar(self, app, create_test_user):
        with app.app_context():
            user = User.lookup('foo@bar.com')
            assert user.avatar

    def test_get_admin(self, app, create_test_user):
        with app.app_context():
            user = User.lookup('foo@bar.com')
    ...
    def test_identity(self, app, create_test_user):
        with app.app_context():
            user = User.lookup('foo@bar.com')
            _identity = user.identity
            assert _identity == user.id

    def test_rolenames(self, app, create_test_user):
        with app.app_context():
            user = User.lookup('foo@bar.com')
            role_name = 'test'
