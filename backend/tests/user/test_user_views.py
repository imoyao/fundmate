...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            {'message': {'json': {'_schema': ['用户名已经存在，请尝试更换用户名后重试。']}}},
        ),
        (
            {'username': 'foobar1', 'password': 'foobar2000', 'email': 'foo@bar.com'},
            {'message': {'json': {'_schema': ['该邮箱已经注册，请检查收件箱或者尝试重新找回密码。']}}},
        ),
        (
...
@pytest.mark.parametrize('data', [({'username': 'test_foo', 'password': 'foobar2000', 'email': 'test_foo@bar.com'})])
...
def test_disable_user(app, client, default_guard, request):
    ...
    # 未设置为admin的用户禁用自身
    token = default_guard.encode_jwt_token(user)
    by_username = {'username': user.username}
    ...
    deny_bad_user_info = {'username': bad_user.username}
    # 使用普通token禁用用户
    ...
    assert bad_user.is_active
    # 通过邮箱禁用
    deny_bad_user_email_info = {'email': bad_user.email}
    # admin禁用普通用户
