...
class TestCategories:
    ...
    @pytest.mark.parametrize('category_data', [({'name': '嘲风优选'}), ({'name': '超越巴菲特'})])
...
class TestManageLabelsOfCollectionsView:
    ...
    def test_patch_labels_of_collections(self, client, auth, bearer_header, create_base_data):
        ...
        is_mine_collection_but_not_all_mine_labels_result = self.patch_labels(
            client, headers=_headers, collection_id=_col_id, data=new_labels_data
        )
...
class TestCategoryDetail:
    ...
    def test_patch_category(self, client, auth, bearer_header):
        ...
        _default_data = {'name': settings.DEFAULT_CATEGORY_NAME}
...
class TestManageCategoriesOfCollectionsView:
    ...
    def test_patch_categories_of_collections(self, client, auth, bearer_header, create_base_data):
        ...
        is_mine_result = self.patch_categories(
            client, headers=_headers, collection_id=_col_id, data=with_def_categories
        )
        logger.info(f'====is_mine_result.json====={is_mine_result.json}====')
        assert is_mine_result.status_code == 205
        is_mine_result_resp = is_mine_result.json
        ...
        is_mine_collection_but_not_all_mine_labels_result = self.patch_categories(
            client, headers=_headers, collection_id=_col_id, data=new_labels_data
        )
