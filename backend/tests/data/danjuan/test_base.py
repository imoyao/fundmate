...
from backend.fundmate.data.danjuan.base import DanJuanFundDetail
...
class TestDanJuanFundDetail:
    ...
    def setup_class(self):
        ...
        :return:
        """
        self.dj_detail = DanJuanFundDetail()
    ...
    def test_parse_first(self):
        assert self.dj_detail.parse_first('买入金额<100.0万') == 1000000
    ...
    def test___parse_both_limit(self, qt_name, p_type, expected):
        assert self.dj_detail._parse_both_limit(qt_name, p_type) == expected

    def test_parse_middle(self):
        test_list = [{'name': '100.0万<=买入金额<500.0万', 'value': '1.2'}, {'name': '500.0万<=买入金额<1000.0万', 'value': '0.8'}]
        assert self.dj_detail.parse_middle(test_list) == [{
    ...
    def test_parse_last(self, range_str, replace_flag, split_signal, expected):
        assert self.dj_detail.parse_last(range_str, replace_flag, split_signal) == expected
    ...
    def test_remove_duplicate_resort(self):
        ...
        assert self.dj_detail.remove_duplicate_resort(a) == [{
            'name': '0.0天<持有期限<7.0天',
            'value': '1.5'
        }, {
            'name': '0.0天<持有期限<30.0天',
