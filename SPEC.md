...
### 3.11.4 ProductDisplay 产品信息复合列组件
...
### 3.11.5 usePageRefresh 全局数据同步组合式函数
...
# 9. 项目四象限路线图 & 完整进度表（2026-07-01 更新）
...
> 多倍贝 暗色模式采用独立的设计规范（详见 `/frontend/design.dark.md`），核心方向：
> - 背景使用深灰层级（非纯黑）
...
**文档结束语**：本文档为 多倍贝 项目唯一权威、完整闭环的长期维护标准，覆盖项目愿景、业务规则、架构设计、UI规范、开发准则、测试要求、进度管理、决策追溯、技术债务、AI编码约束全维度内容。所有规则均来自实战踩坑复盘与标准化落地总结，无模糊定义、无冲突规则、无遗漏约束，可完全支撑项目长期迭代、自主维护、版本复盘、开发接续与项目交接，为全生命周期开发提供统一、唯一、不可私自变更的事实依据。
