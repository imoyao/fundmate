...
**版本**: v3.6
**最后更新**: 2026-06-11
**状态**: P1-09 基金交割单导入收尾完成，支付宝 PDF 解析器上线，全面测试通过，即将开启 P1-10 年化收益率计算
...
| P1-10 | 组合年化收益率计算（IRR + TWR） | ⏸️ 未开始 | ⭐⭐⭐⭐⭐ | 🟡 | Ⅱ | 仪表盘核心指标，数据基础已就绪，下一阶段优先实施 |
...
后续任何会话接续开发，只需携带：
1. 本完整 SPEC 文档（v3.6）
2. 当前进度一句话，如："P1-09 基金交割单导入收尾完成，支付宝 PDF 解析器上线，全面测试通过，即将启动 P1-10 年化收益率计算"
...
| `backend/app/services/importer/parsers/alipay_pdf.py` | **支付宝基金交易 PDF 解析器（新增）** |
...
| `tests/services/importer/test_alipay_pdf_parser.py` | **支付宝 PDF 解析器测试（新增）** |
