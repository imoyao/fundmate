...
- **配置目标绑定层级（2026-05-18 决策）**
    - 配置目标（五笔钱）采用三级优先级：**Ledger 默认目标 → 导入时流水级覆盖 → Position 持仓级最终生效**。
    - 导入预览表格增加“配置目标”列，支持单行修改和批量设置。
    - 持仓页面始终可手动调整配置目标。
- **负债录入**
    - 负债（信用卡、房贷等）属于 `Asset` 表，通过全面盘点页面步骤1的“录入负债/应收款”卡片进入独立表单。
    - 首页“添加资产”按钮可跳转至全面盘点页面。
- **资产简记限制**
    - 首页弹窗记账仅支持股票、基金的买入/卖出操作。
    - 其他操作（分红、定投、复杂转账等）引导至全面盘点页面。
- **导入强制选择账户**
    - 全面盘点页面上传文件前，必须选择一个 `Ledger`（资金容器），不可使用默认值。
...
#### 7. 体验优化需设时间盒
- **经验**：对于非阻塞性 UI 细节，设定 2 小时时间上限；超时则记录到 `IDEAS.md`，延后处理。
- **准则**：拒绝无限度打磨界面细节，优先保障业务功能落地。

#### 8. 资金划转与特殊交易
- **经验**：当前版本暂不支持银证转账等资金划转的自动导入，前端需明确提示用户并禁用勾选。
- **准则**：明确功能边界，不强行兼容未规划业务场景。
...
#### 6.2 P1 任务路线图（2026-05-18 整合四象限优先级矩阵完整版）
| 编号 | 任务 | 状态 | 重要度 | 紧急度 | 所属象限 | 投入执行策略 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| P1-03 | 全面盘点页面（文件导入、预览、去重、行内编辑） | ⚠️ 核心已完成 | ⭐⭐⭐⭐⭐ 核心顶级 | 🔴 高 | Ⅰ 重要且紧急 | 全力收尾，仅修复阻塞性Bug，**冻结新增功能与UI微调** |
| P1-10 | 组合年化收益率计算模块 | ⏸️ 未开始 | ⭐⭐⭐⭐⭐ 核心顶级 | 🟡 中 | Ⅱ 重要不紧急 | 优先启动开发，前置完成历史行情数据铺垫 |
| P1-09 | 清仓持仓历史收益分析 | ⏸️ 未开始 | ⭐⭐⭐⭐ 高阶重要 | 🟡 中 | Ⅱ 重要不紧急 | 等待行情数据完备后有序投入开发 |
| — | 数据层基建（price_history/基金经理数据表填充脚本） | ⏸️ 未开始 | ⭐⭐⭐⭐⭐ 底层核心 | 🟡 中 | Ⅱ 重要不紧急 | 后台异步并行推进，不占用前端开发主线 |
| P1-11 | 基金经理持仓跟踪模块 | ⏸️ 未开始 | ⭐⭐⭐ 实用功能 | 🟢 低 | Ⅱ 重要不紧急 | 中长期规划，先完成基础数据采集留存 |
| P1-08 | 特别关注页面功能增强 | ✅ 基础版本完成 | ⭐⭐⭐ 实用功能 | 🟢 低 | Ⅳ 不重要不紧急 | 现有版本满足使用，Sparkline走势图等功能统一延后迭代 |
| — | 全局UI细节精细化微调（上传区比例、骨架屏动画、弹窗交互优化） | ✅ 全部完成 | ⭐⭐ 锦上添花 | 🟢 低 | Ⅳ 不重要不紧急 | 已达到可用标准，**永久停止投入优化** |
| P1-12 | 高阶数据导入导出增强功能 | ⏸️ 未开始 | ⭐⭐ 远期需求 | 🟢 低 | Ⅳ 不重要不紧急 | 仅用户明确提出需求后再开发，不主动排期 |
| P1-13 | 全端移动端响应式适配 / PWA | ⏸️ 未开始 | ⭐⭐ 远期规划 | 🟢 低 | Ⅳ 不重要不紧急 | 纳入长期版本规划，当前迭代完全搁置 |

好的，现在列出当前最新的工作计划，方便你决定下一步方向。

---

## 📋 当前任务路线图（更新于 2026-05-19）

| 编号 | 任务 | 状态 | 重要度 | 紧急度 | 说明 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **P1-03** | 全面盘点页面收尾 | ⚠️ 核心已完成，细节待自测 | ⭐⭐⭐⭐⭐ | 🔴 高 | 账户选择、配置目标已实现，需自测通过后冻结 |
| **P1-04** | 股票/基金元数据填充 | ✅ 已完成 | ⭐⭐⭐⭐⭐ | — | 脚本已执行，远程搜索可用 |
| **P1-11** | 标签体系架构升级 | ⏸️ 未开始 | ⭐⭐⭐⭐ | 🟡 中 | `tags` 表增加 `entity_type` 字段 |
| **P1-12** | 投资目标 (Objective) CRUD | ⏸️ 未开始 | ⭐⭐⭐⭐ | 🟡 中 | 心理账户概念落地 |
| **P1-13** | 投资目标与持仓关联 | ⏸️ 未开始 | ⭐⭐⭐⭐ | 🟢 低 | 导入/持仓页面可配置归属 |
| **P1-09** | 多维度资产全景视图 | ⏸️ 未开始 | ⭐⭐⭐⭐ | 🟡 中 | 按产品类型/渠道/目标/成员切换视图 |
| **P1-10** | 年化收益率计算 | ⏸️ 未开始 | ⭐⭐⭐⭐⭐ | 🟡 中 | 核心仪表盘指标，依赖价格历史数据 |
...
你想先做哪个？直接告诉我编号，我立刻开始。

##### 象限统一定义
1. **Ⅰ 重要且紧急**：影响核心业务流程、造成功能阻塞、数据错误的任务，48小时内闭环处理
2. **Ⅱ 重要但不紧急**：决定产品长期核心价值、提升投资分析能力的核心功能，固定时间排期落地
3. **Ⅲ 不重要但紧急**：本项目当前无常规此类任务，仅临时环境报错、依赖冲突归入此类，极简快速修复
4. **Ⅳ 不重要也不紧急**：纯视觉美化、极致体验打磨、远期创意功能，统一归档延后，不占用主线开发

##### 重要度星级释义
- ⭐⭐⭐⭐⭐：项目顶层核心能力，直接决定产品定位与核心价值，必须做扎实
- ⭐⭐⭐⭐：完善产品生态的重要功能，补齐使用闭环，做到基础可用即可
- ⭐⭐⭐：提升使用舒适度的辅助功能，限定开发时长，不深度打磨
- ⭐⭐：纯远期规划功能，仅存档记录，当前版本不安排开发

##### 投入策略释义
- 全力投入：不限定时长，保证逻辑严谨、边界场景全覆盖、测试完善
- 适度控制：设定固定时间盒（2~3天），优先完成核心逻辑，交互细节延后
- 最低可用：只做最基础实现，快速上线验证，后续迭代增强
- 冻结：当前版本不再新增功能或做体验微调，仅修复严重Bug

##### 全局开发决策原则
1. 执行优先级：第一象限 ＞ 第二象限 ＞ 第四象限，同象限按照业务依赖、用户价值排序
2. 开发权重排序：数据准确 > 流程顺畅 > 视觉美观
3. 所有新增需求、优化想法必须先完成象限定级，再判定是否投入开发
4. 每周固定一次任务象限重评估，业务状态变更可调整优先级
...
#### 10.1 前端色彩规范（不可妥协）
- 所有前端界面必须从 `src/styles/colors.css` 定义的 CSS 变量中选取颜色，**禁止使用硬编码色值**。
- 语义色（主色、涨跌色、告警等）保持全局统一，涨跌色严格遵循国内金融习惯（涨红跌绿）。
- 标签与分组色板使用项目提供的 12 色莫兰迪色盘，保证视觉协调。
- 完整色彩变量定义见 `src/styles/colors.css`。

---

### 11. 未来优化计划（`IDEAS.md` 摘录）

以下是开发过程中产生的优化想法，已记录在案，**不在当前迭代实现**：
- 资金划转自动归集到现金资产（`Ledger` 模块）
- 基金申购拨出的净值自动获取（需要行情数据）
- 骨架屏呼吸动画的 JS 驱动方案（当前 CSS 方案已够用）
- 全面盘点页面的“对账模式”（差异对比）
- 特别关注页面的 Sparkline 走势图（依赖 `price_history` 表）
- 标签颜色选择器的实时预览
- 表格行内编辑的快捷键支持（Tab 切换下一行）

---

### 12. 当前项目阶段评估

**项目已超出纯 MVP 阶段，进入“可用产品打磨期”。**
- 核心记账、自选、导入三大闭环已形成，产品可交付使用。
- 当前工作重点为修复阻塞性 Bug、保证数据准确性和基本用户体验。
- 后续迭代应遵循“时间盒”原则，控制体验优化的投入比例，优先推进缺失的核心功能（如年化收益率）。

---

### 13. 下一步行动（优先级严格排序）
1. **第一象限收尾**：盘点导入页面遗留阻塞Bug修复，完成后正式冻结该模块
2. **第二象限前置准备**：编写脚本批量填充`price_history`历史行情数据表
3. **核心功能启动**：正式开发 **P1-10 年化收益率计算**（仪表盘核心刚需指标）
4. **并行基建**：完善基金经理、持仓穿透相关底层数据结构
5. 第四象限所有优化需求统一归档，暂停开发
...
| 文件路径 | 作用说明 |
|------|------------|
| `backend/app/main.py` | 项目入口、全局蓝图注册全貌 |
| `backend/app/core/database.py` | 数据库基类、会话工厂、`get_db` 依赖 |
| `backend/app/core/symbol_utils.py` | 全市场证券代码标准化解析工具 |
| `backend/app/services/data_provider.py` | xalpha/AKShare 数据源防腐统一层 |
| `backend/app/domains/positions/views.py` | 持仓核心业务接口 |
| `backend/app/domains/transactions/views.py` | 交易流水筛选分页接口 |
| `backend/app/domains/watchlist/models.py` | 自选体系全部数据模型 |
| `backend/app/domains/watchlist/views.py` | 自选分组、标签、关注全套接口 |
| `src/api/positions.ts` | 前端持仓相关请求封装 |
| `src/api/transactions.ts` | 前端交易流水请求封装 |
| `src/api/watchlist.ts` | 前端自选模块全套API |
| `src/views/transaction/TransactionList.vue` | 交易流水表格+时间线视图页面 |
| `src/views/asset/InventoryImport.vue` | 资产盘点/文件导入核心页面 |
| `src/views/asset/AssetPanorama.vue` | 全资产全景总览页面 |
| `src/components/QuickEntry/TransactionModal.vue` | 手动记账弹窗组件 |

4.  **最新的报错截图或要解决的具体问题**

**本文档是 ShowBuy 项目的唯一事实标准。所有后续开发决策，必须严格遵循本文档所有约束条款，不可私自修改既定设计规范与开发路线。**
