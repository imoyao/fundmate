...
- 产出校验（防假成功，两轮共用）：`scripts/ai_review_verify_guard.js`——**2026-09-18 起（#1581）**该脚本另统计 AI 评论里的「生成标记」类幻觉（`# added` 等固定形态，inline + summary 双通道）并写进 job summary：**只计数、不删除评论、不改变 job 成败**。
