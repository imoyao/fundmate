...
`domains/*/views.py` 的职责边界（决策见 `decisions.md` 2026-09-19 行）：
...
批次 2 候选：`ledgers` 余下视图内业务规则（活期+ 绑定换绑、改名快照刷新、类型变更校验）、
`positions` / `funds` 等同类文件、`ledgers/views.py:29` 的跨域 views import。
