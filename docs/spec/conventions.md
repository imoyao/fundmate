...
### 2.13 事务边界规范（#1609，决策 D30，2026-09-20）

- **单一事务范式（方案 A）**：仓库只保留**一种**事务边界——**事务由用例/视图层持有（一个请求一个事务）**，服务层（`services/**`）与 `BaseRepository` **只 `flush()` 不 `commit()`**。
- **`BaseRepository.save()/delete()` 移除内建 `commit()`**，改为 `flush()`；提交时机由调用方（用例编排层 / 请求边界）统一决定——请求成功末尾 `commit()`、异常 `rollback()`。
- 与 §2.2「视图函数内最多一次显式 `commit()`、多步写入必须整体成功或整体回滚（不得半提交）」对齐：**视图 / 用例层是仓库内唯一允许 `commit()` 之处**；服务方法、Repository 严禁 `commit()`。
- **前置依赖 #1608（会话获取统一）**：事务边界收敛的前提是会话来源统一——`get_db()` / `market_session()` / `user_session()` / `SessionLocal()` 须收敛为单一入口（参见 §4.2 测试数据库隔离准则「禁止直接导入 `SessionLocal`」），否则「一个请求一个事务」无法成立。**#1609 与 #1608 同批推进，不与 #1606 合并执行**（先定范式、再拆视图）。
- **分批收敛，不做一次性大改**：先在 `BaseRepository` 去内建 commit 并配套请求级 unit-of-work（成功提交 / 异常回滚，使既有单服务调用视图无需逐个改写即继续工作），之后按域把多步写操作抽到用例编排层，**每批以 `pytest -p no:xdist` 全量通过为界**。
- **高价值多步写必须锁死回滚**：账户迁移、导入提交、分红再投资等必须补「中途注入异常 → 断言数据完全回滚、源数据不变」的反向用例。
