...
### P3（页面层）
- **概览页短/中/长期落地**：依赖 B1。采用「**行内三连**」而非图2 三张大卡片——综合温度环下方一行小字并列 `短期 30.79° 偏低 ｜ 中期 49° 适中 ｜ 长期 44° 适中`，再补一句结论文案（如「短期情绪偏冷，中期估值适中，长期处于历史低位」）。UI 骨架已就绪（§A 顶部四排均已 SectionHeader + 12 列栅格），只待数据注入。
