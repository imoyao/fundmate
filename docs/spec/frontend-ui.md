...
## 3. 页面规模与工程结构规范（2026-08-24 新增，#980）

本节约束 Vue 页面的**文件规模、目录结构与职责划分**，与视觉规范（design.md）正交。目标：冻结新增上帝页面，存量渐进拆分。

### 3.1 规模上限

- **页面主文件（路由入口 .vue）≤ 400 行**；超过即须按 §3.4 顺序渐进拆分。
- 行数只作触发器不作 KPI：拆分目的是职责清晰，不是凑行数。

### 3.2 职责边界

- 页面主文件只负责**编排**（布局组合、数据下发、事件上抛），不承载业务细节。
- 子组件职责单一、命名清晰；无状态展示组件优先。
...
<page>/
  ...
  components/        # 页面级子组件
  composables/       # 页面级组合式函数（状态/副作用/请求）
  utils/             # 纯函数工具
  constants/         # 常量、枚举、配置
  types/             # 页面级类型
```
...
1. 常量、枚举、配置 → `constants/`（零风险）
2. 纯函数、工具方法 → `utils/`（与页面结构无关）
3. 状态逻辑、副作用、请求 → `composables/`
4. 无状态展示组件 → `components/`（输入 props，输出 UI）
5. 按页面区域拆容器组件（待结构稳定后）

### 3.5 抽象克制原则

- 逻辑只在一个页面使用 → 拆**局部**组件/函数；
- 同样逻辑/UI 出现 **2 次以上** → 再抽象为通用组件/公共模块；
- 禁止"看似通用、实际只服务一个页面"的伪通用组件。

### 3.6 执行约束

- 新页面/新功能必须遵守本节，Code Review 将「是否遵守」作为检查项；
- 存量超限页面按「最近改动频繁 → Bug 集中 → 核心链路」优先级排序，每迭代还 1~2 个，不追求一次完美。
