...
> 本文档固化多多贝（duoduobei.com）旗下各站点的职责边界、导流顺序、身份互通与数据隔离决策。
...
| 应用站 多多贝应用站（FundMate） | `app.duoduobei.com` | 应用层（终点） | Nuxt/Vue 前端 + 后端 | 用户注册/登录/使用产品，**最终落脚地**；**记账唯一入口** |
| 探市 | `app.duoduobei.com/explore` | app 子路由 | app 内路由（免登录沙盒） | 研究页，引流到 app 转化 |
| 叽咕（新，基估宝） | `jigu.duoduobei.com` | 过渡产品线 | Next.js 静态导出 | 基金估值追踪，过渡期可用产品 |
| 叽咕（旧，jigu.masantu.com） | `jigu.masantu.com` | 待下线 | 旧部署 | 旧用户数据，计划下线 |
...
```
                     duoduobei.com  ── 营销层（主站 + 人格测试 H5）
                          ...
                          ▼
     ┌──────────────────────────────────────────────────┐
     │   app.duoduobei.com  ── 应用层（终点 / 多多贝应用站 FundMate）   │
...
- **过渡产品线（叽咕 / 基估宝）**：现在能用就留着，不投精力二次开发，未来退役、流量归并主站导流链；**仅承担展示与导流，不提供记账，记账唯一入口在多多贝应用站（FundMate）**。
...
叽咕(jigu.duoduobei.com) ──（保留过渡）→ 未来退役 → 流量归并主站
```

- 叽咕**不排入「导流到 app」的链路**作为一环，它是平行产品线；退役时其域名 301 到主站对应入口。
- 「免费开始 / 使用产品」主按钮直接跳 `https://app.duoduobei.com`。

## 四、身份互通与数据隔离

### 4.1 叽咕 ↔ app（同主域，必选 SSO）

- 叽咕与 app **指向同一个 Supabase project**（改 env 不改代码）。
- 用户登叽咕 = 在 app 注册，反之亦然；`user_id` 隔离，天然共享用户池。
...
- SSO 必选范围 = **jigu ↔ app（多多贝应用站 FundMate）**（Supabase 父域 cookie）。
- FeedLog 走独立数据库 + 可选 JWT 桥，**不纳入 Supabase SSO**，复杂度可控。

## 五、叽咕新旧迁移策略

### 新叽咕（jigu.duoduobei.com，指向主 Supabase）

- 复杂度低：与 app 同 project，用户/数据天然共享，无需迁移。
- 上线新叽咕，同步下线 `jigu.masantu.com`。

### 旧叽咕（jigu.masantu.com，旧 Supabase project）
...
- 旧站保留只读一段时间作为窗口期，再 301 到新叽咕或主站。
- 跨子域 cookie 方案就绪后，新叽咕与 app 登录互通自然成立；旧用户迁过来后同样适用。

## 六、部署形态

- 主站 / 人格测试 H5：静态站，部署到 **EdgeOne Pages**（仓库 `duoduobei-web`，根目录即产物，`/personality` 为子路径）。
- 叽咕：Next.js `output: export` → `out/`，EdgeOne Pages，绑定 `jigu.duoduobei.com`。
- app：多多贝应用站（FundMate）前端，EdgeOne，绑定 `app.duoduobei.com`。
