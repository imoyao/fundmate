...
> ⚠️ **易腐烂内容**：本文件随修复进展频繁变化。最后核实日期：**2026-08-04**。每条债务修复后，须将状态更新为"✅ 已修复"并注明版本号；请勿删除历史条目（保留可追溯）。
...
| **行业拥挤度分母（全A中位PB）在受限网络下不可达，整组标灰** | 🟠 高 | 行业拥挤度 `crowding_pct` 的分母 = 全A中位PB，默认走 `ak.stock_a_all_pb()`（legulegu）；无本地缓存时若 legulegu 不可达，代码退化为东财实时PB兜底（`push2.eastmoney.com`），再不可达则返回 stale 占位（`crowding_pct=None`），不抛异常、不阻塞主链路。当前沙箱/受限网络下 legulegu 与东财实时源均 `RemoteDisconnected`，导致 `market_pb_series()` 取不到分母。 | **已知问题（2026-08-05 复现 + 修复脚本）**：新增 `backend/scripts/prefetch_all_pb.py`，在**网络正常的环境**执行一次即可把历史PB落盘到 `app/services/thermometer/cache/all_pb.csv`，之后受限环境也能复用分母恢复拥挤度。当前环境（2026-08-05 验证）网络连不上上述源，需在有正常网络（或代理）的机器上跑一次。代码本身无 bug，纯数据源可达性问题。 |
