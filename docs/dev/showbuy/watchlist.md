...
## ShowBuy 投资记账工具 - 需求设计文档合集

**版本**：v2.1（最终整合版）
**日期**：2026年5月13日
**说明**：本文档整合自选股功能、记账增强（简记与全面盘点）、资产复盘三大模块的需求设计，并已根据多轮架构评审完成最终优化。本文档自包含，不引用外部文档。
...
| 版本 | 日期 | 修订内容 |
|------|------|----------|
| v1.0 | 2026-05-06 | SPEC.md 初始版本，定义核心数据模型与 MVP 闭环 |
| v2.0 | 2026-05-12 | 整合三大模块需求设计；完成架构评审与务实降级 |
| v2.1 | 2026-05-13 | watchlist 状态机重构：清仓改为动态推导、归档移除、新增特别关注(bookmarked)；清仓分析功能设计；清仓后自动移除替代自动归档；字段命名规范化 |
...
1.  **统一管理**：支持A股、港股、美股等多市场场内资产（股票、ETF、可转债）与场外基金的统一代码标准化和一站式管理
...
| 已清仓 | 有交易记录 + 无持仓 + 清仓日期距今 ≤ 30天 + `bookmarked = FALSE` | 普通自选页面（临时分组） |
| 特别关注 | `bookmarked = TRUE` | 特别关注页面 `/starred` |

**清仓后 30 天自动移除规则**：

-   资产清仓后，自动出现在"已清仓"分组
-   30天内，用户可操作：点击"特别关注"按钮 → `bookmarked = TRUE`，资产移至特别关注页面
-   30天后未标记特别关注的资产：自动从自选列表移除。数据完整保留在 `transactions` 和 `cleared_cycles` 表中，清仓分析页面不受影响
...
-   **多格式代码输入**，自动标准化：
    ...
    -   A股：`SH600519`、`600519.SH`、`600519` → `SH.600519`
...
| | 已清仓 | ✗ | ✗ | 清仓后30天内 + 无持仓 + `bookmarked = FALSE` | 资产 |
...
-   "已清仓"：清仓后 ≤ 30天 + `bookmarked = FALSE` → 自动显示。**30天后未标记特别关注的资产自动从自选列表移除**
...
-   **手动添加**：在"已清仓"分组中，点击资产行右侧的"特别关注"按钮
    -   弹出气泡：已加入特别关注。你希望什么时候提醒复盘？选项：1个月后 / 3个月后 / 暂不提醒
...
**基准选择器**：页面顶部，默认沪深300。可选：上证指数、创业板指、恒生指数、标普500。

**清仓后涨幅列**：P1阶段灰色显示"需要价格历史数据"，引导用户补充数据。
...
-   **导入**：P1阶段支持 ShowBuy 自有格式和纯文本代码列表。东方财富、同花顺、雪球等跨平台模板放 P2 按需支持
...
-   P1阶段仅包含：经理姓名、简介、手动关联的基金列表（无任职回报等专业数据）
...
| 阶段 | 周数 | 交付内容 |
|------|------|----------|
| **P0** | 2-3周 | 数据模型建表；资产添加/删除/搜索/批量（含场内/场外自动识别+手动修正）；代码标准化；分组体系（系统分组动态渲染+自定义分组）；标签管理；置顶；拖拽排序；手动价格/净值更新；视图切换及自适应表头；持仓盈亏联动；简记弹窗唤起；特别关注标记与页面 |
| **P1** | 2-3周 | 自动行情/净值数据源；清仓分析页面（按时间线+按产品汇总）；基金经理基础信息与关联；经理详情面板；变更提醒（页面内）；关注理由与投资笔记；异动提醒（页面内高亮）；导入导出（自有格式+纯文本列表）；清仓后30天自动移除；深色模式 |
...
4.  **清仓后默认不保留**：30天后自动移除，只有用户主动标记的才留在"特别关注"
