...
> 适用：任何人接手「多多贝」乖离率（bias）数据为空的问题时，照此手册排查，避免重复踩坑。
> 创建：2026-08-02 ｜ 状态：**已闭环（2026-08-05）**——bias 数据源已改为直连（`bias/direct_feeds.py`，腾讯行情/东财 push2his），`jobs.py` `SKIP_BIAS=False` 已放开；本手册记录旧排查过程，仅作历史参考。行业拥挤度侧 baostock 主源切换仍未闭环（见 `docs/spec/tech-debt.md` 第 64 条）。
