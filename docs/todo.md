...
7. 账户不够用（每个用户免费 10 个，不够需要购买）

8. 总金额的时间轴，还有账户之间转移

建议可以支持不同账本之间的转出/入资金（基金转换）

#### 记账

**基金现金**分红在账本里面怎么记录？
  买了一只基金，不算分红亏了 10%，算上分红赚了 10% 。
...
类似腾讯自选股，但更进一步。
个人对腾讯自选股有以下不满：
1. 无法记录分红。买了一只基金，不算分红亏了 10%，算上分红赚了 10% 。
2. 无法记录已经卖出的部分。自选股强制要求持有份额为正数，导致无法将历史收益加入计算。
3. 无法对投资的个股年化收益和个人总投资年化收益进行计算。

接触投资理财的时间不长，对各类 APP 基本两眼一抹黑。
不过很清楚的是，我不需要常见的收支记账工具，只想要单纯的投资收益分析。

能不能增加数据导出功能？跨平台。Android APP 能使用 Android design 那就更好了，当然没有也只能接受。
