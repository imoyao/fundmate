...
1. **无真实问题就不评论**——不对中性变更、也不对「这行被新增/删除/改写」做复述式点评；宁可不评论，也不要为「覆盖每处改动」凑数。
2. **先读 PR 标题与描述**——理解本次改动目标，禁止提出与目标相反的建议。
3. **给 `suggestion` 前先打开文件核对**——禁止建议「已经就是现状」的写法，禁止 `suggestion` 为 `null`/空（会破坏代码）。
...
- **「注释 / 文档的删除·改写·精简」不是缺陷（PR #1383 高频，29 条几乎全为此类）**：注释、docstring、文档段落被删除、改写、收敛，**本身不构成缺陷**，**严禁**以「Removed comments should be retained / 应保留原注释 / 注释丢失」为由提意见。唯一可提的情形：删除后**丢失了代码无法自证的、必要的强约束**（如「此字段禁止直接改，须走 X」「此处必须持锁」），且代码中没有等价说明——即便如此也只给 [次要]。把「旧行为描述」更新为「新行为描述」（旧描述已不成立）是**正确清理**，不得逐条点评；**若本次 PR 的目标就是「清理过时注释/文档」，则对该 PR 的注释改动整体不再提任何意见**。
- **禁止「复述 diff」式中性点评（inline 尤其）**：不要输出「Added comment …」「Removed line …」「Updated X …」这类只是描述「这行变了」的评论。inline 评论**只允许出现在确有缺陷/风险的行**；对纯注释、纯文档、纯文案、等价替换，**没有可验证的问题就不评论**。
- **建议前必须核对现状——禁止建议「已存在的写法」**：给出 `suggestion` 前，务必在 checkout 中打开该文件，确认你的建议**是否已经就是现状**。典型误报：对一行已是 `x: Engine = Factory.create(DOMAIN_USER)` 的代码，建议「应显式写成 `x: Engine = Factory.create(DOMAIN_USER)`」。**当替换内容等于现状、或你拿不准正确写法时，只写文字说明，不要输出 `suggestion` 代码块**；**尤其禁止 `suggestion` 内容为 `null`/空**（会把正常代码置空，属破坏性误报）。
- **禁止逆 PR 意图的建议**：评论前先读 **PR 标题与描述**，理解本次改动目标。**严禁**提出与目标相反的建议（例：PR 目标是「把缺省库统一为 `invest.db`」，就不得建议「改回 `invest.dev.db`」；PR 目标是「删除过时约定」，就不得建议「恢复该约定」）。若认为方向本身有问题，在 summary 用 [次要] 说明理由，而非逐行反向 suggestion。
- **有意保留的历史值 / 兼容候选不是缺陷**：脚本候选列表、注释里保留的旧文件名/旧值/旧取值（如「历史缺省 `invest.dev.db`，存量机器可能仍有，保留扫描」、`CANDIDATE_URLS` 末尾的旧文件名）、向后兼容分支，均为**有意为之**；除非能证明其导致**实际功能错误**，否则不得提意见。
- **缺省值 / 常量取值变更本身不是缺陷**：`_DEFAULT_*`、`.env` 缺省、脚本缺省的修改属常规演进；仅当能证明「无任何变量的全新环境下会指向错误库 / 破坏既有数据」时才提，且须给出复现路径。**不得**仅凭「dev 与 prod 配置不一致」这类泛化担忧提 [主要]。
- **diff 阅读纪律（再次强调）**：你用的是补丁文本，`-` 行是**被删除的旧内容**、`+` 行才是**当前内容**。判定一律以 `+` 行为准；不得把 `-` 行当现况，也不得把「`-` 旧行 → `+` 新行」的等价替换误读为「删除 / 回归」。
- **约定/风格类误报（PR #1395 三连实证，共 40 条行内意见几乎全为复述 diff 或误读约定）**：下列情形**禁止**作为问题提出（本仓库有明确既定约定，bot 常在不知约定的情况下按通行规范反向建议）：
  - **「中文注释 / 中文 docstring / 中文字段 `comment=` 应改英文」**：本仓库**中文优先**（见下方「输出语言」节），模型 `comment=`、docstring、`#` 注释、测试断言消息**普遍用中文**，是既定约定而非缺陷。**不要**以「应保持英文文档风格」为由提问题；仅当同文件内中英混排且明显破坏可读性时才至多 [次要]。
  - **「SQLAlchemy 模型 mixin 声明顺序」**：本仓库模型统一为 `class X(Base, PrimaryKeyMixin, TimestampMixin, FamilyScopedMixin)`（全仓 14 处一致）。**不要**建议调整继承顺序（如把 `FamilyScopedMixin` 前移）——该顺序即仓库约定，改动无收益且会与其余模型不一致。
  - **「测试 import 风格 / 常量命名」**：单测沿用标准 `import pytest` / `import loguru` 与模块级常量（如 `FUND_CODE`），**不要**建议改成 `from pytest import *`（反模式）、要求把 `import x` 拆成 `from x import y`、或以「常量应全大写」为由提问题；同理「函数命名应使用下划线」若被测函数本已是 `_make_job` 形式，即为误读。
  - **「历史记录被当成待办」**：`docs/spec/decisions.md` 是 **append-only 决策日志**，表格每行是**已拍板的结论**（含"当时背景/问题"的叙述）。**不要**把里面的历史问题描述读成"当前待办"并建议去修；`changelog.md`、`tech-debt.md` 条目同理并非本次 diff 引入的问题。
- **只审本 PR 实际改动的行——不得越出 diff 范围（PR #1401 高频）**：审查范围**严格等于**本 PR 的 `+`/`-` 行。**严禁**对**未被 diff 触及的既有行**提意见，包括但不限于「该既有内容可能破坏 X」「应重新评估该决定」「建议补充说明该既有段落」。判定方法：提任何意见前先确认该行号落在本 PR 的 diff 区间内；若某行在 diff 中只作为**上下文**出现（无 `+`/`-` 前缀），即**不得**对它提意见。
- **append-only 历史记录文件（`decisions.md` / `changelog.md`）只审新增行（PR #1401 高频，9 条全误报实证）**：这两个文件是**只追加（append-only）的历史档案**，其既有行记录的是**已经拍板并已生效的过往决策**，不是本次提案、也不是待办建议。典型误报：对既有决策行提「The decision to remove X may break existing code / 该决定可能导致数据不一致 / 可能让用户难以理解」——这些决策早已合并且经测试验证。**禁止**对 append-only 文件的既有行做任何「可能导致破坏 / 应重新考虑」的点评；只审本次新增的那一行（且新增行只需核对：与既有编号体系一致、引用的编号真实存在、内容自洽）。
- **Markdown 文档正文不是「代码注释」——禁止要求删除文档内容，禁止语气审查（PR #1401 高频，15 条全误报实证）**：文档（`*.md`）中的**正文段落、引用块（`>`）、列表、标题**是交付内容本身，**不是**代码注释。下列情形**严禁**提出：
  - **「注释中的内容与代码无关，应删除」**——文档正文讨论的从来就不是代码，此类意见为纯误报；尤其**禁止**建议删除「教训来源」「反例」「背景」这类实证依据段落（它们正是本仓库文档的价值所在）。
  - **语气审查**——禁止以「语气过于强硬 / 应使用更中性的表达」为由提意见。本仓库文档风格即为**断言式、禁止模糊表述**（见 `AGENTS.md` / `SOUL.md`），「必须 / 禁止 / 不得」是**规范用语而非语气问题**。
  - **反向要求压缩既有规范**——不得以「缺少具体描述」为由要求把清单式条款（如准入四问）逐条展开成长段说明；**若 PR 的目标本身就是精简/收敛，则对该 PR 的详略改动一律不提意见**（接上文「禁止逆 PR 意图的建议」）。
  - **`suggestion` 格式约束**——禁止把 Markdown 正文包进 json 代码块再作为 suggestion 输出（会破坏文档渲染）。
  - 文档类 PR 的正确审查重点：**引用的路径/编号是否真实存在**、**跨文件口径是否自相矛盾**、**新增内容是否与既有规范冲突**、**链接与锚点是否可解析**。
