...
| 7a | **货币基金每日收益链路验证（验收标准，2026-08-09 补录）**：验证 (1) 货币基金记账后万份收益落库 `money_fund_daily_worth.nav_per_10k`；(2) 每日收益计算链路**打通且结果正确**（日收益 = 持有金额 × 当日万份收益 ÷ 10000；前端持仓收益/账户收益展示入口已接入，与基金净值/股票行情同链路可展示）；(3) 与支付宝等平台同期收益对账一致。当前代码无任何消费者读取 `nav_per_10k`（`calculators.py` 只查 `DailyWorth`），**必须先实现或确认展示入口再验收** | 0.5d | 6 |
