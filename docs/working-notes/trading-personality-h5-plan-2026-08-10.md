...
| 数据源 | 万得全 A 881001.WI 仅作概念锚点（付费）；实际用 akshare/东财/腾讯免费源；Supabase 免费额度覆盖种子期 30~100 人 |
...
行为签名引擎（五维：出击/纪律/专注/切换/逆向，min-max 归一化）
        ...
        └── 表面 B：实测行为 → 后端 behavior 领域计算 5 行为因子
                前端：资产复盘 asset-review 雷达图（welcome 仅「已更新」入口卡）
        │
        └── 闭环：按 contact 反查，自报 vs 实测对比（后期）
```

- **落点修订（2026-08-13，按分层原则）**：雷达图**常驻资产复盘页 asset-review**（该页已规划「综合评分雷达图」，行为体检并轨于此），**不放 welcome 首页**——行为签名是月度周期的派生洞察，静态数据不占首页寸土寸金的位置；welcome 仅在有新扫描结果时显示「行为体检已更新 →」入口卡（点入跳复盘页），保持首页 live。
- **前期**：复盘页雷达图数据源 = 盲测结果（submissions 表 norm_scores），用户授权码关联后展示。
- **后期**：数据源切换为行为分析 API（`/api/behavior/signature/`），同一雷达组件、同一归一化引擎，前端只换数据源。
- **前端落地**：`asset-review`（资产复盘页）增加雷达图卡片（vue-echarts RadarChart），数据源按阶段切换；welcome 仅渲染「已更新」入口卡（由月度扫描完成事件触发，无更新不占位）；H5 结果页独立实现同款雷达图（原生 JS 或轻量 echarts CDN）。
...
| 5 | 万得全 A 881001.WI 付费 | 低 | akshare/东财/腾讯免费源替代（temperature 模块已验证） |
...
| P3 | 复盘集成 | `asset-review` 雷达图（前期接盲测，后期接行为分析）+ welcome「已更新」入口卡 | P1/P2 | 雷达图数据正确渲染 |
...
- 2026-08-13：雷达图落点修订（welcome 首页 → asset-review 复盘页 + welcome「已更新」入口卡），依据页面分层原则「会动的首页放，不动的进报告」；行为签名按月度周期扫描，不做实时。
