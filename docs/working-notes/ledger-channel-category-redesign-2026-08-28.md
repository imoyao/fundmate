...
- 参考值（枚举，不再增减）：`stock`(股票账户) / `fund`(基金账户) / `bank`(现金账户) / `property`(实物资产) / `e_account`(基金 E 账户汇总，导入自动建) / `family`(家庭账户)。
...
- 目的：标识系统级聚合账本（如基金 E 账户），从用户账户列表默认隐藏。
- 行为：`Boolean`，默认 `False`；`True` 时列表查询过滤（前向兼容未来聚合账本）。
- 参考值：`True` 仅基金 E 账户等系统聚合账本。
...
1. **沪 A/深 A/京 A 不建模为子账本**：属交易席位（market 差异），由持仓 `market`(CN_A/CN_B/CN_N) 处理，建子账本仅增复杂度无收益。
