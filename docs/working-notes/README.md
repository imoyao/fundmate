...
| 文件 | 内容 |
|---|---|
| `data-refresh-scheduling-plan-2026-09-13.md` | **待拍板（v2）**：#1467 第一阶段的对照 + **全量数据项盘点**（探市 20 资产 / 温度计 9 源 + JSL 保活 / 账本持仓 / 元数据 D1~D16）+ **三类结构性问题**（`run_all_jobs` 漏 job、探市页数据未进任何调度、交易日口径一刀切致海外数据停更）+ **事实核证 9 条**（`git log -S` 证明是遗漏而非有意；`index_valuation` 0 行=从未执行、源实测可用；**探针发现 2 个汇率资产数据停在 2023-11-10**）+ **10 项决策各带推荐** + 验收标准与回滚 + 本机可靠性评估 + PR 拆分 + 60 分钟议程。**注：§3-A 的 job 计数与 F6/D10 已由下行实施计划回填更正** |
| `market-snapshot-persist-plan-2026-09-13.md` | **探市数据底座 · 方案 B 实施计划（#1460，已实施并真机跑通）**：A/B/C 三方案对比与选型理由、**复用 `market_multi_items` 的字段映射**（不新建表）、落什么/不落什么（只落算好的结果，不落收盘序列）、`market_snapshot` job 与读库路径设计、`MARKET_OVERVIEW_SOURCE=live\|db` 开关与回滚、验收标准（**真机实测：15 行 / 15 成功 / 0 失败 / 11.9s，读库 p95 16ms**）、**顺带修掉四项**：F1（`execution_plan` 实漏 **5** 个 job 含 `channel_link`，附完整性回归测试）、**F6 根因查明**（`currency_boc_sina` 默认区间被上游硬编码 2023-03-04~2023-11-10，非源坏，补传日期即修好，D10 降级方案作废）、D4 最小可用日历维度（`cn`/`none`）、**V8 并发 abort 缺陷**（§8.1，已升级为平台级硬约束 D24）。另记录 1 项未改的周末汇率显示瑕疵 |
