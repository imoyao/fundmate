...
> 参考站：fundfof.com（基金研究/FOF 分析平台，数据驱动，与多多贝手动记账定位不同）
...
> 结论：你们的数据底座比 v2 假设**完整得多**。fundfof 的"研究层"（全市场排名/经理/回测/AI 选基）本就不该抄；可借鉴的是它的**分析展示层**，且必须只算你们**自己的数据**。
