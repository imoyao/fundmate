...
| 2 | **答非所问 / 意图跟随** `#1718` | 必须在 S2 后：改的 `agent_loop.py` 与 S2 同文件，先动必冲突 | 待开工（你最痛的点，优先于历史栏） |
| 3 | **历史会话栏** `#1719` | 必须在 S2 后：读 `agent_session` 表 + 改 `api/agent.ts`/`index.vue` 同文件 | 待开工 |
| 4 | **S3 护栏与成本** | 与 2/3 **可并行**（动 `safety/` + 配额，不同文件）；`#1294` 配额迁出内存归此 | 待做 |
| 5 | **S4 可观测与评估** | 内含 `#1718` 的**回归位**（30 条评估集把「答得对」变成可验证，防止改完又悄悄坏） | 待做 |
| 6 | `#1714` 可中断（停止按钮） | 依赖 S2 的服务端会话（取消标志要挂在会话上） | OPEN |
| 7 | `#1712` 结构化叙事渲染 | 与 2~5 **可并行**（纯前端渲染，不动记忆层） | OPEN |
| 8 | **S5 协议层 → S6 面试收口** | — | 待做 |
...
| 卡 | 触发条件（到点即做，做完在本表打勾） |
|---|---|
| `#1721` StaticPool 假阳性 | **S4 建 `agent_trace` 表之前必做**——新表同样会踩「未提交 INSERT 被 close 回滚」 |
| `#1722` 全量测试提速 | **S4 之前做一次** `--durations` 量化（评估集要反复跑全量，届时 36 分钟会变成瓶颈）；或下次全量超 40 分钟 |
