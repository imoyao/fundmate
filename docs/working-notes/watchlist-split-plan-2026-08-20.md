...
## 实况备忘（2026-08-24，#980 推进计划步骤 2）

- `index.vue` 实测 **563 行**（A+B 完成，落在 ~550 预期内；按 `frontend-ui.md` §3 新规 400 行上限属「存量超限」，按 §3.6 渐进原则可暂不再拆，收益递减）。
- **死样式清理已于 2026-08-22 完成**（style 区仅存迁移说明注释 :652/:777-779，疑似失效规则已删净）——#980 推进计划步骤 3 勾选滞后，实际已完成。
- C 类进度：#993 持久化（localforage）已前置完成；#992/#991/#990 按依赖序待推进。
- 工程规范落地：`docs/spec/frontend-ui.md` 新增 §3「页面规模与工程结构规范」（≤400 行上限、目录结构、抽象克制原则），新页面强制遵守，存量按优先级渐进。
