...
> ⚠️ **文档状态：历史记录（部分已过时）**。本文档归集了本轮代码审查、V1 退役清除、接口可用性评估、测试瘦身与 FastAPI 解耦规划的全部工作记录，避免散落于仓库根目录 `docs/`。
> 原始 6 份文档已按章节合并，内容忠实保留。
> **重要修正**：本文 §1.1 将「Supabase 上云」判定为合规风险，**该结论已被 D3（2026-08-07）推翻**——项目已明确「架构必须上云」，核心账本（用户域）生产落 Supabase，市场域落 Turso，「永不上云」旧承诺取消。请勿以 §1.1 的「不上云」立场作为当前规范依据；当前数据库架构事实标准见 `docs/dev/db-data-domain.md`（2026-08-18）。
...
### 1.1 云端后端（Supabase）与「本地优先、不上云」硬性规范冲突  【合规风险 · 已被 D3 推翻，2026-08-07】

> ⚠️ 本节结论已失效：当时把 Supabase 上云当作「违反 SPEC 不上云规范」的合规风险。自 D3（2026-08-07）起，项目已明确**架构必须上云**，核心账本（用户域）生产落 Supabase 属既定设计，不再视为风险。本节作为历史记录保留。
